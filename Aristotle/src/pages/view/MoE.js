...
import { queryMixingOfExperts } from "../../apis/moe.api";

const ChatRoom = () => {
  const { mixtureOfExperts } = useContext(ChatContext);
  const { messages, addNewMessage } = mixtureOfExperts;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryMixingOfExperts({ query: textQuery });
      queryResult.match({
        Ok: (data) => {
