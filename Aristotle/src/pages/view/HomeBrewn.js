...
import { queryHomeBrewn } from "../../apis/homeBrewn.api";

const ChatRoom = () => {
  const { homeBrewn } = useContext(ChatContext);
  const { messages, addNewMessage } = homeBrewn;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryHomeBrewn(textQuery);
      queryResult.match({
        Ok: (data) => {
      ...
      >
        ...
        >
          <InputUpload
            ...
            placeholder={"Message Home Brewn"}
            isShowUpload={false}
          />
        </Box>
      </Box>
