...
import { queryRetail } from "../../apis/retail.api";

const ChatRoom = () => {
  const { retail } = useContext(ChatContext);
  const { messages, addNewMessage } = retail;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryRetail(textQuery);
      queryResult.match({
        Ok: (data) => {
