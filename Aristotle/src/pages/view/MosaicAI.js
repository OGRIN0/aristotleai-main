...
import { queryMosaicAI } from "../../apis/MosaicAI.api";
import Chat from "../../component/Chatbot/Chat";


const ChatRoom = () => {
  const { MosaicAI } = useContext(ChatContext);
  const { messages, addNewMessage } = MosaicAI;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryMosaicAI(textQuery);
      queryResult.match({
        Ok: (data) => {
