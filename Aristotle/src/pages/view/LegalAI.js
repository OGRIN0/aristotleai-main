...
import { queryLegalAI } from "../../apis/legalAI.api";

const ChatRoom = () => {
  const { legalAI } = useContext(ChatContext);
  const { messages, addNewMessage } = legalAI;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryLegalAI(textQuery);
      queryResult.match({
        Ok: (data) => {
