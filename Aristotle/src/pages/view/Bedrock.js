...
// import { Container, Paper, Stack, Fab, Box } from "@mui/material";
...
//     try {
//       const gaiRes = await axios.get(`http://localhost:8000/bedrock/${inputValue}`);
...
//         {
//           from: "llm",
//           message: gaiRes.data.completion,
//         },
//       ]);
//     } catch (error) {
...
//         </Paper>
//       </Stack>
//       <Box sx={{ position: "fixed", bottom: 16, right: 16, bgcolor:"transparent" }}>
...
import { queryBedRock } from "../../apis/bedRock.api";

const ChatRoom = () => {
  const { bedRockAI } = useContext(ChatContext);
  const { messages, addNewMessage } = bedRockAI;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryBedRock(textQuery);
      queryResult.match({
        Ok: (data) => {
