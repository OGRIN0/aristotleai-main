...
import { queryHybrid } from "../../apis/hybrid.api";

const ChatRoom = () => {
  const { hybrid } = useContext(ChatContext);
  const { messages, addNewMessage } = hybrid;
  ...
  const handleSubmitQueryOrUpload = useCallback(async () => {
    ...
    if (query.trim()) {
      ...
      setQuery("");
      const queryResult = await queryHybrid(textQuery);
      queryResult.match({
        Ok: (data) => {
